-- Primero eliminamos todas las políticas existentes para evitar conflictos
...
DROP POLICY IF EXISTS "Servicio backend tiene acceso total" ON usuarios;

-- Aseguramos que RLS esté habilitado
ALTER TABLE usuarios ENABLE ROW LEVEL SECURITY;

-- Política para el servicio backend (debe ser la primera)
...
BEGIN
    ...
    IF policy_count != 7 THEN
        RAISE EXCEPTION 'No se crearon todas las políticas esperadas. Se encontraron % políticas.', policy_count;
    END IF;
END $$; 